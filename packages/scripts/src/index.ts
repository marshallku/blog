function toggleTheme() {
    ...
    const currentTheme = document.documentElement.dataset.theme as (typeof themes)[number];
    const nextTheme = themes[(themes.indexOf(currentTheme) + 1) % themes.length];

    document.documentElement.dataset.theme = nextTheme;
    document.documentElement.style.colorScheme = nextTheme === "dark" ? nextTheme : "light";
